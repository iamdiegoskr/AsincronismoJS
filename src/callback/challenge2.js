...
fetchData(api, (error1, allCharacteres) => {
    // si error, matamos retornando un error
    if (error1) return console.error(error1);
    // luego buscamos en la api el id de Rick
    fetchData(api + allCharacteres.results[1].id, (error2, onlyCharacter) => {
        // si error, matamos retornando un error
        if (error2) return console.error(error2);
        // por ultimo la consulta a la api que contiene su dimension
        fetchData(onlyCharacter.origin.url, (error3, ubication) => {
            // si error, matamos retornando un error
            if (error3) return console.error(error3);

            // mostramos los resultados :) 
            console.log(allCharacteres.info.count);
            console.log(onlyCharacter.name);
            console.log(ubication.dimension);

            // rutas de las peticiones en orden
            console.log(api);
            console.log(api + allCharacteres.results[0].id);
            console.log(onlyCharacter.origin.url);
